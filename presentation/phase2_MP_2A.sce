...
int num_trials = 40;  # user enters initial value in dialog before scenario
...
#last sub! find the sequences in the choice history & reward history and get the left and right
sub
    array<int,1> choice_counting (array<int,1>& choiceHis, array<int,1>& rewardHis, int num)
begin
	 array<int> choice[2]={0,0};
    int lenChoice = choiceHis.count();
    int lenReward = rewardHis.count();
    if num==0 then
       loop
          int i=1
       until
       ...
       array<int> seqChoice[num];
       array<int> seqReward[num];
       seqChoice=slice(choiceHis, lenChoice-num+1, lenChoice);
       seqReward=slice(rewardHis, lenReward-num+1, lenReward);
       ...
       until
          j>lenChoice-num
       begin
          if slice(choiceHis, j, j+num-1) == seqChoice then
             if slice(rewardHis, j, j+num-1) == seqReward then
   ...
   nolick.present();
	...
	if i<=4 then
        ...
        until
            j>4
        begin
            choiceCount = choice_counting(agentChoiceHistory, rewardHistory,j);
...
ofile1.open("Algo2test.txt", false);
